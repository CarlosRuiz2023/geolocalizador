                                                                ...
                                                                if (result.rows.length === 0) {
                                                                    ...
                                                                    if (result.rows.length === 0) {
                                                                        ...
                                                                        query = `
                                                                            ...
                                                                            ;
                                                                        `;
                                                                        values = [direccionParsed.NOMVIAL, direccionParsed.NUMEXTNUM1, direccionParsed.MUNICIPIO];
                                                                        const result = await pgClient.query(query, values);
                                                                        for (let i = 0; i < result.rows.length; i++) {
                                                                            result.rows[i].scoring = {
                                                                                fiability: 20,
                                                                        ...
                                                                        `;
                                                                            ...
                                                                            `;
                                                                                ...
                                                                                if (result.rows.length === 0) {
                                                                                    ...
                                                                                    END AS x_centro
                                                                                    FROM carto_geolocalizador
                                                                                    WHERE unaccent(nombre_vialidad) LIKE '%' || $3 || '%'
                                                                                    AND codigo_postal = $1 
                                                                                    AND unaccent(municipio) = $2
                                                                                    ;
                                                                                `;
                                                                                    values = [direccionParsed.CP, direccionParsed.MUNICIPIO, direccionParsed.NOMVIAL];
                                                                                    const result = await pgClient.query(query, values);
                                                                                    for (let i = 0; i < result.rows.length; i++) {
                                                                                        result.rows[i].scoring = {
                                                                                            fiability: 10,
