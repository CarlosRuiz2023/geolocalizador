            ...
            if (result.rows.length === 0) {
                ...
                if (result.rows.length === 0) {
                    ...
                    query = `
                        ...
                        ;
                    `;
                    values = [direccionParsed.COLONIA, direccionParsed.CP, direccionParsed.MUNICIPIO, direccionParsed.ESTADO, direccionParsed.NUMEXTNUM1];
                    const result = await pgClient.query(query, values);
                    for (let i = 0; i < result.rows.length; i++) {
                        result.rows[i].scoring = {
                            fiability: 40,
                                    ...
                                    if (result.rows.length === 0) {
                                        ...
                                        if (result.rows.length === 0) {
                                            ...
                                            query = `
                                                ...
                                                ;
                                            `;
                                            values = [direccionParsed.COLONIA, direccionParsed.CP, direccionParsed.MUNICIPIO, direccionParsed.NUMEXTNUM1];
                                            const result = await pgClient.query(query, values);
                                            for (let i = 0; i < result.rows.length; i++) {
                                                result.rows[i].scoring = {
                                                    fiability: 30,
                                                    ...
                                                    if (result.rows.length === 0) {
                                                        ...
                                                        if (result.rows.length === 0) {
                                                            ...
                                                            query = `
                                                                ...
                                                                ;
                                                            `;
                                                            values = [direccionParsed.COLONIA, direccionParsed.CP, direccionParsed.ESTADO, direccionParsed.MUNICIPIO];
                                                            const result = await pgClient.query(query, values);
                                                            for (let i = 0; i < result.rows.length; i++) {
                                                                result.rows[i].scoring = {
                                                                    fiability: 30,
                                                            ...
                                                            if (result.rows.length === 0) {
                                                                ...
                                                                for (let i = 0; i < result.rows.length; i++) {
                                                                    ...
                                                                    try {
                                                                        // Calcular la distancia de Levenshtein
                                                                        distance = levenshteinDistance(result.rows[i].nombre_vialidad, direccionParsed.CALLE);
                                                                        // Calcular la similitud como el inverso de la distancia de Levenshtein
                                                                        maxLength = Math.max(result.rows[i].nombre_vialidad.length, direccionParsed.CALLE.length);
                                                                    } catch (error) {
                                                                        // Calcular la distancia de Levenshtein
                                                                        distance = levenshteinDistance(result.rows[i].nombre_vialidad, direccionParsed.COLONIA);
                                                                        // Calcular la similitud como el inverso de la distancia de Levenshtein
                                                                        maxLength = Math.max(result.rows[i].nombre_vialidad.length, direccionParsed.COLONIA.length);
                                                                    }
