...
// Aplicable solo en caso de llevar los campos CALLE MUNICIPIO y NUMEXTNUM1
                ...
                if (result.rows.length !== 0) {
                    ...
                    if (result.rows.length === 0) {
                        ...
                        query = `
                            ...
                            END AS x_centro
                            FROM carto_geolocalizador
                            WHERE (((CAST(l_refaddr AS INTEGER) <= $1 AND CAST(l_nrefaddr AS INTEGER) >= $1)
                        ...
                        if (result.rows.length !== 0) {
                            ...
                            /* if (result.rows.length === 0) {
                                ...
                                for (let i = 0; i < result.rows.length; i++) {
                                    ...
                                    if (result.rows[i].l_refaddr <= direccionParsed.NUMEXTNUM1 && result.rows[i].l_nrefaddr >= direccionParsed.NUMEXTNUM1 && result.rows[i].scoring.calle > 80) {
                                        result.rows[i].scoring.numero_exterior += 100;
                                        result.rows[i].scoring.fiability += 25;
                                        // Asignar la imagen final que recibira dicha direccion
                                        result.rows[i].imagen = 'punto';
                                    }
                                    else if (result.rows[i].l_refaddr >= direccionParsed.NUMEXTNUM1 && result.rows[i].l_nrefaddr <= direccionParsed.NUMEXTNUM1 && result.rows[i].scoring.calle > 80) {
                                        result.rows[i].scoring.numero_exterior += 100;
                                        result.rows[i].scoring.fiability += 25;
                                        // Asignar la imagen final que recibira dicha direccion
                                        result.rows[i].imagen = 'punto';
                                    }
                                    else if (result.rows[i].r_refaddr <= direccionParsed.NUMEXTNUM1 && result.rows[i].r_nrefaddr >= direccionParsed.NUMEXTNUM1 && result.rows[i].scoring.calle > 80) {
                                        result.rows[i].scoring.numero_exterior += 100;
                                        result.rows[i].scoring.fiability += 25;
                                        // Asignar la imagen final que recibira dicha direccion
                                        result.rows[i].imagen = 'punto';
